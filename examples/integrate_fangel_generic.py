...
# 0. Load electromagnetic data (FANGEL_AVG.h5)
...
# Use RAW DATA instead of AVERAGED DATA. (73209 soundings)
useRaw = False
if useRaw:
    files = ig.get_case_data(case=case, filelist=['FANGEL_RAW.h5'], showInfo=2)
    f_data_h5 = files[0]
...
# get prifile line indices
buffer = 25  # meters
X, Y, LINE, ELEVATION = ig.get_geometry(f_data_h5)
X_profx = [X.mean(), X.mean()]
Y_profx = [Y.min(), Y.max()]
id_linex, distances, segment_ids = ig.find_points_along_line_segments(
    X, Y, X_profx, Y_profx, tolerance=buffer
)

X_profy = [X.min(), X.max()]
Y_profy = [Y.mean(), Y.mean()]
id_liney, distances, segment_ids = ig.find_points_along_line_segments(
    X, Y, X_profy, Y_profy, tolerance=buffer
)
...
id_line, distances, segment_ids = ig.find_points_along_line_segments(
    X, Y, X_prof, Y_prof, tolerance=buffer
)

plt.plot(X, Y, 'k.', label='All data')
plt.plot(X[id_linex], Y[id_linex], 'ro', label='Profile X')
plt.plot(X[id_liney], Y[id_liney], 'bo', label='Profile Y')
plt.plot(X[id_line], Y[id_line], 'go', label='Profile diagonal')
plt.xlabel('X (m)')
plt.ylabel('Y (m)')
plt.axis('equal')
plt.legend()
plt.title('Data locations and selected profile lines')
plt.show()  
...
f_prior_h5 = ig.prior_model_layered(N=N,lay_dist='chi2', NLAY_deg=4, RHO_min=10, RHO_max=300, f_prior_h5='PRIOR_N%d.h5' % N)
...
try:
    ig.plot_feature_2d(f_post_h5,im=1,iz=5, key='Median', uselog=1, cmap='jet', hardcopy=hardcopy)
    plt.show()
except:
    pass

try:
    ig.plot_feature_2d(f_post_h5,im=1,iz=30, key='Median', uselog=1, cmap='jet', hardcopy=hardcopy)
    plt.show()
except:
    pass

try:
    ig.plot_feature_2d(f_post_h5,im=1,iz=50, key='Median', uselog=1, cmap='jet', hardcopy=hardcopy)
    plt.show()
except:
    pass
