...
# Nice profile
X1 = 544000; X2= 543550; Y1=6174500; Y2=6176500;
...
# inflateNoise be be tested for values, 1,2,5,10
inflateNoise = 4
#if inflateNoise != 1:
if inflateNoise > 0:
    gf=inflateNoise
...
WELLS_compressed.append(W_compressed)
...
W['lithology'] = ['Moræneler', 'Grus', 'Moræneler', 'Grus', 'Grus', 'Miocæn sand', 'Miocæn ler']
...
W_compressed = copy.deepcopy(W)
W_compressed['depth_top'] =     [0,  8, 15,   20, 23.5, 45]
W_compressed['depth_bottom'] =  [8, 15, 20, 23.5,   45, 46]
...
W['Y'] = 6175822.76
W['name'] = 'DAU02'
WELLS_compressed.append(W.copy())
...
f_prior_data_identity_h5 = ig.prior_data_identity(f_prior_data_h5, im=2, id=2, doMakePriorCopy=True)

#%%
P_single=0.8
for iw in np.arange(len(WELLS_compressed)):
    print("considering well %d: %s" % (iw+1, WELLS_compressed[iw]['name']))

    W = WELLS_compressed[iw]
    ...
    i_use = np.zeros((nd, 1))  # Use all data points
    d_obs = np.zeros((nd,nclass, nm))*np.nan
    r_data=10
    r_dis=100
    w_combined, w_dis, w_data, i_use_from_func = ig.get_weight_from_position(f_data_h5, X_well, Y_well, r_data=r_data, r_dis=r_dis, doPlot=True)
    #for ip in id_line:
    T_all = 1/w_dis
    # set all T>101 to 100  
    T_all[T_all>100] = 100
...
# %% TEST
Dall, idx = ig.load_prior_data(f_prior_data_identity_h5)
Mall, idx = ig.load_prior_model(f_prior_data_identity_h5)
...
lithology_obs = WELLS[0]['lithology_obs']
depth_top = WELLS[0]['depth_top']
depth_bottom = WELLS[0]['depth_bottom']
...
f_post_h5 = 'post_%s_inflateNoise%d.h5' % (fileparts[0], inflateNoise)

f_post_h5 = ig.integrate_rejection(f_prior_data_identity_h5, 
                                f_data_h5, 
                                f_post_h5, 
                                N_use = N_use, 
                                showInfo=0, 
