...
1. `@streamlit@ig_data.py` – Allow analyzing HDF5 files (with extension 'h5') in the current folder. files should be sleected in a listbox, and shoud show relevant info about the HDF5 file. It should also try to identify whether the HDF5file is DATA, PRIOR file (with prior model and/or data), or a POSTERIOR file (with the 'i_use' data set).
